...
<template>
  <v-navigation-drawer app fixed clipped class="w-400" width="400" v-model="drawer">
    <v-card class="!flex !flex-col pa-0 fill-height">
      ...
      <v-card-text class="fill-height overflow-y-scroll">
        <div v-for="(_, index) in lines" class="flex flex-row items-center my-6" :key="index">
          <v-checkbox class="w-20" density="compact" v-model="checks[index]" hide-details />
          <v-btn icon flat density="compact"
            @click="() => (lines[index] -= step) && handleValueChange(index)"><v-icon>mdi-minus</v-icon></v-btn>
          <v-slider class="w-1/1" v-model="lines[index]" hide-details="" max="10" :step="step" thumb-label="always"
            @update:modelValue="() => handleValueChange(index)"></v-slider>
          <v-btn icon flat density="compact"
            @click="() => (lines[index] += step) && handleValueChange(index)"><v-icon>mdi-plus</v-icon></v-btn>
          <v-btn icon flat class="ml-3" @click="() => deleteLine(index)"><v-icon>mdi-delete</v-icon></v-btn>
        </div>
      ...
      <v-card-text>
        ...
        <v-btn class="mt-3" block @click="handleClickRandom">Random</v-btn>
        <v-btn class="mt-3" block @click="handleClickKeepSumRandom">Keep Sum Random</v-btn>
      </v-card-text>
    </v-card>
  </v-navigation-drawer>
</template>